...
export const SlideInput = memo((p: {
  slide: Slide;
  ...
  const selectedLineRef = useRef<{ lineNumber: number | null, content: string, anchor: number | null }>({ lineNumber: null, content: "", anchor: null });
  ...
  useEffect(() => {
    const handleKeyDown = async (event: KeyboardEvent) => {
      ...
      } else if ((event.ctrlKey || event.metaKey) && event.shiftKey && (event.key === "x" || event.key === "X") && refProgress.current > 0) {
        event.preventDefault()
  ...
  const startToastProgress = (toastRef: any) => {
    ...
    intervalProgressUpdate = setInterval(() => {
      refProgress.current = refProgress.current + 0.10;
  ...
  const renderToastDescription = () => <div>
